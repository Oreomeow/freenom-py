...
class FreeNom:
    ...
    DOMAIN_INFO_REGEX = (
        '<tr><td>(?P<domain>[^<]+)<\/td><td>[^<]+<\/td><td>[^<]+<span class="[^"]+">(?P<days>\d+)['
        '^&]+&domain=(?P<id>\d+)"'
    )
    ...
    def run(self) -> list:
        ...
        for info in domain_info_match:
            ...
            if int(days) > 14:
                print(
                    f"FreeNom domain {domain} can not renew, days until expiry is {days}"
                )
    ...
    def login(self) -> bool:
        data = {"username": settings.FN_ID, "password": settings.FN_PW}
        headers = {**self.headers, "Referer": "https://my.freenom.com/clientarea.php"}
